...
4. Vibe runs the reviewer automatically when the plan and required worker are complete. Continue on `VERDICT: FAIL`; finish only when the final non-empty reviewer line is exactly `VERDICT: PASS` and is backed by fresh evidence.
