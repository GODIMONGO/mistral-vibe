...
- Soft foreground timeout is opt-in: `git_bash(background=false, hard_timeout=false)` returns a live session when `timeout_seconds` expires. Use it only when the process is intentionally meant to remain inspectable.
...
**Examples:**
- Bounded build: `git_bash(command="npm run build", timeout_seconds=300)` kills a hung build after five minutes so the agent can recover.
